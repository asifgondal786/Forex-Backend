...
# ── Phase 14 Patch B — Forex + AI env validation ─────────────────────────
import os as _os
...
def startup_snapshot_phase14() -> dict:
    """Call this inside your existing startup_snapshot() and merge the result."""
    return {
        ...
        "ai_routes_enabled":   _os.getenv("AI_ROUTES_AVAILABLE", "false").lower() in {"true", "1", "yes"},
    }

def validate_forex_env(raise_on_missing: bool = False) -> list:
    """Returns list of missing required keys. Optionally raises ValueError."""
    missing = [k for k in FOREX_API_KEYS_REQUIRED if not _os.getenv(k, "").strip()]
