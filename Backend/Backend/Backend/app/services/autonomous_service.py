...
async def _notify_risk(user_id: str, pair: str, drawdown_pct: float, threshold_pct: float):
    """Call this when drawdown threshold is breached to fire high-priority alerts."""
    await _dispatcher_as.dispatch(
        user_id    = user_id,
        event_type = "risk",
        payload    = {
            "pair":      pair,
