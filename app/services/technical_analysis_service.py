...
async def _fetch_closes(pair: str, interval: str = "1h", count: int = 50) -> List[float]:
    """Fetch closing prices via shared cache."""
    from app.services.twelve_cache import fetch_timeseries
    data = await fetch_timeseries(pair, interval=interval, outputsize=count)
    if not data:
        return []
    try:
        ...
        logger.warning("Technical data parse failed for %s: %s", pair, exc)
        return []
