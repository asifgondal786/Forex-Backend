...
async def get_market_prices(
    ...
    # 1b. In-memory fallback cache (when Redis unavailable)
    if not redis_client:
    ...
    # 4b. Store in memory cache (when Redis unavailable)
    if not redis_client and quotes:
        _mem_cache[cache_key] = {
            "ts": _time.monotonic(),
            "data": {"prices": [q.model_dump() for q in quotes], "fetched_at": now_iso},
        }
