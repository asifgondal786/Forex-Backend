...
async def analyze_market_with_gemini():
    ...
    rates = await forex_data_service.get_rates()
    news = await forex_data_service.get_forex_news()

    return await forex_data_service.get_market_snapshot()
...
async def predict_price_movements(pair: str, historical_data: List[Dict]):
    ...
    return await forex_data_service.get_ohlc(pair=pair)
...
async def get_features_status(user_id: str):
    ...
    try:
        ...
        # Get market data
        from . import forex_data_service
        sentiment = await forex_data_service.get_sentiment()
